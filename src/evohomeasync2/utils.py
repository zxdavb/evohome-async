...
@click.option("--cache-tokens", "-c", is_flag=True, help="Cache access tokens.")
...
def cli(
    ...
    ctx.obj[SZ_EVO] = EvohomeClient(username, password, debug=bool(debug), **tokens)
...
def _dump_tokens(evo: EvohomeClient) -> None:
    ...
    tokens = {
        ...
        SZ_ACCESS_TOKEN_EXPIRES: evo.access_token_expires,
    }
    with NamedTemporaryFile(
        mode="w", prefix=".evo-cache", suffix=".json", delete=False
    ) as fp:
...
def _load_tokens() -> dict[str, dt | str]:
    ...
    with NamedTemporaryFile(
        mode="r", prefix=".evo-cache", suffix=".json", delete=False
    ) as fp:
        try:
